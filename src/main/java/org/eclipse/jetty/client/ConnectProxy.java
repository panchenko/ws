...
public class ConnectProxy extends ProxyConfiguration.Proxy {
    private static final Logger LOG = Log.getLogger(HttpProxy.class);

    public ConnectProxy(String host, int port) {
        super(new Origin.Address(host, port), false);
    }
    ...
    @Override
    public URI getURI() {
        return URI.create(new Origin(HttpScheme.HTTP.asString(), getAddress()).asString());
    }
    ...
    private class CreateTunnelPromise implements Promise<Connection> {
        ...
        private void tunnelSucceeded(EndPoint endPoint) {
            try {
                ...
                org.eclipse.jetty.io.Connection newConnection = connectionFactory.newConnection(endPoint, context);
